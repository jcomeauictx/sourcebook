% adapted from https://www.overleaf.com/learn/latex/Sections_and_chapters
% for testing ucs, do:
%  \usepackage{ucs}
%  \usepackage[utf8x]{inputenc}
\documentclass{book}
% sloppy or emergencystretch needed for markdown processor
\sloppy  % helps READMEs and listings lines wrap
\pdfminorversion=3  % for ingramspark, to eliminate transfer curves
\usepackage{tocloft}  % for making more room for section numbers
\setlength{\cftsecnumwidth}{3em}  % e.g. 1001.123
\setlength{\cftsubsecnumwidth}{4.5em} % 100.1010.123
\usepackage[letterpaper,portrait,margin=1in]{geometry}
\usepackage{parskip}  % vertical spacing rather than indentation of paragraphs
\usepackage{xstring}  % string length (\StrLen) for ifthenelse actions
\usepackage[pipeTables,tableCaptions]{markdown}
\usepackage{verbatim}  % allow reading in files with \verbatiminput{filename}
\usepackage{listings}  % for source code listings
\usepackage{xcolor}  % for use of colored keywords etc.
\usepackage{textcomp}  % for cents sign and other obscure characters
\usepackage[T1]{fontenc}  % for guillemets and other obscure characters
\usepackage{CJK}  % CJK characters, also helps avoid non-CJK UTF-8 errors
\input{endpage.tex}  % make sure to end on even page
\title{$BOOKTITLE}
\author{$AUTHOR}
\date{$REVISION}
\begin{document}
\maketitle
\tableofcontents
